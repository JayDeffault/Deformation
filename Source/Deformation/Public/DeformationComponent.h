...
UENUM(BlueprintType)
enum class EDeformationBoneNormalAxis : uint8
{
	X,
	NegativeX,
	Y,
...
public:
	...
	/** Use each bone's local normal axis as the dent direction; ideal for "hedgehog" deformation bones. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Deformation|Direction")
	bool bUseBoneNormalForDeformation = true;

	/** Local bone axis used as the inward deformation normal when bUseBoneNormalForDeformation is enabled. */
...
private:
	...
	FVector ResolveBoneNormalDirectionCS(FName BoneName) const;
