...
void UCustomCarPhysicsComponent::BuildRuntimeMeshFromStatic()
{
    ...
    PhysicsMesh.RebuildCollisionPlanes();

    VisualMeshState.Initialize(Vertices, Triangles);
    VisualMeshState.BuildSpatialHash(20.0f);
    VisualMeshState.RecalculateNormalsAll(CachedNormals);
...
void UCustomCarPhysicsComponent::UpdateRuntimeMesh(bool bFullRebuildNormals)
{
    ...
    if (bFullRebuildNormals) VisualMeshState.RecalculateNormalsAll(CachedNormals);
    else if (DirtyVertices.Num() > 0) VisualMeshState.RecalculateNormalsPartial(DirtyVertices, CachedNormals);

    TArray<FVector2D> UV0; UV0.Init(FVector2D::ZeroVector, VisualMeshState.Vertices.Num());
...
void UCustomCarPhysicsComponent::SimulateFrame(float Dt)
{
    ...
    if (bDebugDraw && GetWorld())
    {
        ...
        const int32 TriCount = FMath::Min(FMath::Max(MaxDebugCollisionTriangles, 0) * 3, PhysicsMesh.Triangles.Num());
        for (int32 i = 0; i + 2 < TriCount; i += 3)
        {
            const int32 A = PhysicsMesh.Triangles[i];
            const int32 B = PhysicsMesh.Triangles[i + 1];
            const int32 C = PhysicsMesh.Triangles[i + 2];
            if (!PhysicsMesh.Vertices.IsValidIndex(A) || !PhysicsMesh.Vertices.IsValidIndex(B) || !PhysicsMesh.Vertices.IsValidIndex(C))
            {
                continue;
            }

            const FVector WA = ActorTransform.TransformPosition(PhysicsMesh.Vertices[A]);
            const FVector WB = ActorTransform.TransformPosition(PhysicsMesh.Vertices[B]);
            const FVector WC = ActorTransform.TransformPosition(PhysicsMesh.Vertices[C]);
            DrawDebugLine(GetWorld(), WA, WB, FColor(255, 128, 0), false, 0.03f, 0, 0.5f);
            DrawDebugLine(GetWorld(), WB, WC, FColor(255, 128, 0), false, 0.03f, 0, 0.5f);
            DrawDebugLine(GetWorld(), WC, WA, FColor(255, 128, 0), false, 0.03f, 0, 0.5f);
        }
    }
...
void UCustomCarPhysicsComponent::HandleCarCollisions()
{
    ...
    for (const TPair<int32,int32>& Pair : Pairs)
    {
        ...
        const FCollisionContact Contact = FCollisionSolver::SolveConvexSAT(
            FProxyConvex{AComp->GetOwner()->GetActorTransform(), AComp->PhysicsMesh.Vertices, AComp->PhysicsMesh.Triangles},
            FProxyConvex{BComp->GetOwner()->GetActorTransform(), BComp->PhysicsMesh.Vertices, BComp->PhysicsMesh.Triangles});
...
void UCustomCarPhysicsComponent::ProcessDeformationQueue()
{
    ...
    {
        TArray<int32> PhysicsDirty;
        PhysicsMesh.ApplyDeformationEvent(DeformationQueue[EvtIdx], MaxDeform, PhysicsDirty);

        TArray<int32> VisualDirty;
        VisualMeshState.ApplyDeformationEvent(DeformationQueue[EvtIdx], MaxDeform, VisualDirty);
