...
- `SyncPhysicsBodiesToPoseableBones`: enabled on the pawn by default; after final poseable bone offsets are applied by a hit/recovery refresh, matching PHAT body transforms are rebuilt from the deformed poseable bone transform while preserving the body's original relative offset to that bone, so collision/debug bodies stay attached to the visual dents instead of hanging in their old place.
- `KeepPhysicsBodiesAttachedToBones`: enabled by default; on component tick it reapplies only PHAT body transforms from already-deformed poseable bones, without recalculating mesh deformation, so Chaos cannot leave kinematic bodies behind at the old source-skeleton pose.
